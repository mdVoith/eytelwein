...
def _convert_surcharge_angles(
    equivalent_slope_angle_DIN22101_rad: Optional[float] = None,
    surcharge_angle_ISO5048_rad: Optional[float] = None,
) -> dict:
    """
    Convert between equivalent slope angle (DIN 22101) and surcharge angle (ISO 5048).

    Parameters:
    equivalent_slope_angle_DIN22101_rad (Optional[float]): The equivalent slope angle according to DIN 22101 in radians.
    surcharge_angle_ISO5048_rad (Optional[float]): The surcharge angle according to ISO 5048 in radians.

    Returns:
    dict: A dictionary containing both angles with keys 'DIN22101' and 'ISO5048' in radians.
    ...
    if (
        equivalent_slope_angle_DIN22101_rad is not None
        and surcharge_angle_ISO5048_rad is None
    ):
        # Store the provided equivalent slope angle
        converted_angles["DIN22101"] = equivalent_slope_angle_DIN22101_rad
        # Convert the equivalent slope angle to the surcharge angle and store it
        converted_angles["ISO5048"] = (
            _convert_equivalent_angle_of_slope_to_surcharge_angle(
                equivalent_slope_angle_DIN22101_rad
            )
        )
    # Check if only the surcharge angle is provided
    elif (
        surcharge_angle_ISO5048_rad is not None
        and equivalent_slope_angle_DIN22101_rad is None
    ):
        # Convert the surcharge angle to the equivalent slope angle and store it
        converted_angles["DIN22101"] = (
            _convert_surcharge_angle_to_equivalent_angle_of_slope(
                surcharge_angle_ISO5048_rad
            )
        )
        # Store the provided surcharge angle
        converted_angles["ISO5048"] = surcharge_angle_ISO5048_rad
...
def _convert_equivalent_angle_of_slope_to_surcharge_angle(
    ...
    """
    Convert the equivalent slope angle (DIN 22101) to the surcharge angle (ISO 5048).

    Parameters:
    equivalent_slope_angle_rad (float): The equivalent slope angle according to DIN 22101 in radians.

    Returns:
    float: The surcharge angle according to ISO 5048 in radians.
    """
...
def _convert_surcharge_angle_to_equivalent_angle_of_slope(
    ...
    """
    Convert the surcharge angle (ISO 5048) to the equivalent slope angle (DIN 22101).

    Parameters:
    surcharge_angle_rad (float): The surcharge angle according to ISO 5048 in radians.

    Returns:
    float: The equivalent slope angle according to DIN 22101 in radians.
    """
    # Calculate the tangent of the angle in radians
