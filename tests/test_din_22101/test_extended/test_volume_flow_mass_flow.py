...
class TestConvertSurchargeAngles:
    ...
    def test_convert_surcharge_angles(self):
        result = convert_surcharge_angles(
            equivalent_slope_angle_DIN22101=Quantity(20.0, u.degree)
        )
        assert result["DIN22101"]["value"] == 20.0
        assert result["ISO5048"]["value"] == pytest.approx(28.633, rel=1e-2)

    def test_convert_surcharge_angles_other_way(self):
        result = convert_surcharge_angles(
            surcharge_angle_ISO5048=Quantity(20.0, u.degree)
        )
        assert result["ISO5048"]["value"] == 20.0
        assert result["DIN22101"]["value"] == pytest.approx(13.639, rel=1e-2)
