...
class TestConvertSurchargeAngles:
    ...
    def test_convert_surcharge_angles(self):
        result = _convert_surcharge_angles(
            equivalent_slope_angle_DIN22101_rad=math.radians(20.0)
        )
        assert result["DIN22101"] == math.radians(20.0)
        assert result["ISO5048"] == pytest.approx(math.radians(28.633), rel=1e-2)

    def test_convert_surcharge_angles_other_way(self):
        result = _convert_surcharge_angles(
            surcharge_angle_ISO5048_rad=math.radians(20.0)
        )
        assert result["ISO5048"] == math.radians(20.0)
        assert result["DIN22101"] == pytest.approx(math.radians(13.639), rel=1e-2)
